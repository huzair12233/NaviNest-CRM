...
export async function dealFormOptions() {
  const [leads, properties, owners, team] = await Promise.all([
    ...
    db.property.findMany({ orderBy: { createdAt: "desc" }, take: 200, select: { id: true, code: true, title: true, salePrice: true, rent: true, listingType: true } }),
  ...
  return {
    ...
    properties: properties.map((p) => ({
      value: p.id,
      label: `${p.title} · ${inr(p.listingType === "RENT" ? p.rent : p.salePrice)}`,
    })),
